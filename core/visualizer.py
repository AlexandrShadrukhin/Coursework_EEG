    ...
    def plot_spectrum(self, data, sampling_rate, channel_names, fig=None):
        with self.performance_monitor.measure("Визуализация спектра"):
            ...
            for i in range(n_channels):
                ...
                ax.set_xlabel('Частота, Гц', fontsize=11, labelpad=6)
                ax.set_ylabel('Мощность', fontsize=11, labelpad=6)
                ax.set_title(f'{channel_names[i]}', fontsize=12, fontweight="bold", pad=6)
                ax.grid(True, color="#334155", alpha=0.6, linewidth=0.8)
                ax.set_xlim(0, 40)

            fig.suptitle('Спектры мощности по каналам', fontsize=15, fontweight="bold", color="#e5e7eb")
